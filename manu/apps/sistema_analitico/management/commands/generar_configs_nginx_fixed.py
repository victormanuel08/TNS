...
# -*- coding: utf-8 -*-
"""
Comando FIXED para generar configuraciones de Nginx con valores reales del servidor.
Basado en verificación manual del 2025-12-04:
- Servicio PM2: Eddeso
- Puerto Frontend: 3006
"""
...
FRONTEND_PORT_DEFAULT = 3006
NGINX_DIR_DEFAULT = "/etc/nginx"
OUTPUT_DIR_DEFAULT = "/etc/nginx/sites-available"


class Command(BaseCommand):
    help = 'Genera configuraciones de Nginx para dominios de EmpresaDominio (VALORES FIJOS: Eddeso, puerto 3006)'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--nginx-dir',
            type=str,
            default=NGINX_DIR_DEFAULT,
            help=f'Directorio base de Nginx (default: {NGINX_DIR_DEFAULT})',
        )
        parser.add_argument(
            '--output-dir',
            type=str,
            default=OUTPUT_DIR_DEFAULT,
            help=f'Directorio donde crear las configuraciones (default: {OUTPUT_DIR_DEFAULT})',
        )
        parser.add_argument(
            '--frontend-port',
            type=int,
            default=FRONTEND_PORT_DEFAULT,
            help=f'Puerto del frontend Nuxt (default: {FRONTEND_PORT_DEFAULT} - Eddeso)',
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Generador de Configuraciones de Nginx (VALORES FIJOS)")
        self.stdout.write("=" * 80)
        self.stdout.write(f"Servicio PM2: {PM2_SERVICE_NAME}")
        self.stdout.write(f"Puerto Frontend: {frontend_port}")
        self.stdout.write("=" * 80)
        self.stdout.write("")
        ...
        for dominio in sorted(list(dominios_excluidos)[:10]):  # Mostrar solo primeros 10
            archivo = mapeo_nginx[dominio]['archivo']
            self.stdout.write(f"      - {dominio} → {archivo.name}")
        if len(dominios_excluidos) > 10:
            self.stdout.write(f"      ... y {len(dominios_excluidos) - 10} más")
        self.stdout.write("")
        ...
        for dominio in sorted(list(dominios_empresa)[:10]):  # Mostrar solo primeros 10
            self.stdout.write(f"      - {dominio}")
        if len(dominios_empresa) > 10:
            self.stdout.write(f"      ... y {len(dominios_empresa) - 10} más")
        self.stdout.write("")
        ...
        for dominio in sorted(dominios_a_configurar):
            ...
            if dry_run:
                ...
                self.stdout.write(f"      Contenido (primeras 10 líneas):")
                ...
                try:
                    ...
                    self.stdout.write(self.style.WARNING(f"      Ejecuta: sudo python manage.py generar_configs_nginx_fixed"))
...
# Generado por: python manage.py generar_configs_nginx_fixed
# Servicio PM2: {PM2_SERVICE_NAME}
